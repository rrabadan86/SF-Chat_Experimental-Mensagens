...
from evo_agendamento.orchestrator import _confirm_message, turma_fechada
...
# ─────────────────────────────────────────────────────────────────────────
#  DESMARCAR aula experimental de LEAD (idProspect) — a SoFIA usa quando a lead
#  avisa que não pode ir. simular:true (padrão) só acha a matrícula. Cancela via
#  change-status (status=2 = falta justificada, reversível).
# ─────────────────────────────────────────────────────────────────────────
...
#  na nova numa ÚNICA chamada. Pré-checa a turma nova (existe/aberta/com vaga)
#  ANTES de desmarcar. Ordem: desmarca (gera reposição) → matricula; se a
#  matrícula falhar, REVERTE o cancelamento (status=0).
# ─────────────────────────────────────────────────────────────────────────
...
def api_book_sofia():
    ...
    try:
        res = book_experimental(name=nome, when=when, email=email, phone=telefone,
                                forcar_novo=forcar_novo, evo=evo)
