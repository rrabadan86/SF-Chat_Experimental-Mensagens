...
## Confirmação para a aluna (fila / outbox)

A confirmação da aula deve sair pelo **WhatsApp do Studio (8550-8065)**, que é uma automação
de WhatsApp Web (Edge/Puppeteer) — **não** a API do ZEE (que só envia pelo 996847251). Por isso
o job **não envia** a confirmação: ele **enfileira** num arquivo JSONL (`STUDIO_OUTBOX_FILE`,
padrão `confirmacoes_outbox.jsonl`) e o **bot do Studio** lê essa fila e envia pelo 8550-8065.

Cada linha do outbox:
```json
{"ts":"2026-07-07T18:13:39","contactId":"50adab22-...","name":"Juliana Rabadan","phone":"6281718205","when":"2026-07-10 16:15","message":"Oi, Juliana! ...","status":"pending"}
```
O bot do Studio deve: ler as linhas `status:"pending"`, enviar a `message` para o `phone`
(pelo 8550-8065), e marcar como enviado (ex.: reescrever com `status:"sent"` ou mover para outro
arquivo) para não reenviar.

Canal configurável em `STUDIO_CONFIRM_CHANNEL`: `outbox` (padrão), `zee` (sai do 996847251) ou `off`.
