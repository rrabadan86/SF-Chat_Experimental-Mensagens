...
class ZeeClient:
    ...
    def list_threads(self, since=None, max_pages=50):
        """Lista as conversas recentes PAGINANDO (o /threads devolve 20 por página,
        das mais novas para as mais antigas). Assim não perde ninguém quando há muita
        conversa (ex.: durante uma campanha).

        since: string ISO em UTC (ex.: '2026-07-14T16:00:00.000Z'). Se informado, para
        de paginar quando a página já só tem conversas anteriores a esse instante.
        max_pages: teto de segurança de páginas a buscar.
        """
        todos, vistos = [], set()
        for page in range(0, max_pages):     # /threads é indexado a partir de 0 (page 0 = mais novas)
            batch = self._request("GET", "/threads", params={"page": page})
            ...
            for t in batch:
                ...
                if tid and tid in vistos:
                    continue                      # dedup entre páginas
                if tid:
                    vistos.add(tid)
                todos.append(t)
                novos += 1
                sd = (t or {}).get("startDate") or ""
                if oldest is None or sd < oldest:
                    oldest = sd
            if novos == 0:                        # página repetida (param ignorado) -> para
                break
            if len(batch) < 20:                   # última página
                break
            if since and oldest and oldest < since:  # já passou da janela de tempo
                break
        return todos
