/**
 * app.js — o formulário de agendamento.
 *
 * A página já chega montada do servidor: textos, locais e perguntas estão no
 * HTML. Este arquivo só dá comportamento ao que já existe — navegar entre os
 * passos, buscar os horários livres e enviar o pedido.
 *
 * Nada de regra de negócio aqui: os horários vêm de /api/horarios (que consulta
 * o Google) e a validação de verdade acontece no servidor.
 */
(function () {
  'use strict';
  ...
  if (!$('#agendar')) return;                  // a seção pode estar oculta
  ...
  var estado = { locais: [], loc: null, grade: null, dia: null, hora: null, dados: {}, enviando: false };
    ...
    if (!r.ok) {
      if (!corpo.erro) {
        corpo.erro = r.status === 413
          ...
          : 'O servidor respondeu ' + r.status + '. Tente de novo em instantes.';
      }
      var erro = new Error(corpo.erro);
  ...
  function lerLocais() {
    var caixa = $('#locais-json');
    try { return JSON.parse(caixa.textContent); } catch (e) { return []; }
  }

  function ligarLocais() {
    estado.locais = lerLocais();
  ...
  function localAtual() {
    return estado.locais.find(function (h) { return h.id === estado.loc; }) || { nome: '', duracaoMin: 0 };
  }

  /* ------------------------------------------------ dias e horários */

  async function carregarHorarios() {
    var hospital = localAtual();
    ...
    $('#hintDia').textContent = hospital.nome + ' — consultas de ' + hospital.duracaoMin + ' minutos.';

    if (!estado.grade.dias.some(function (d) { return d.livres > 0; })) {
      $('#slots').innerHTML = '<p class="erro-slot">Não há horários livres nos próximos dias neste local. ' +
        'Tente outro local ou fale com a recepção.</p>';
      return;
    }
  ...
  function montarRevisao() {
    var h = localAtual(), d = estado.dados, anos = idade(d.nascimento);
    $('#review').innerHTML =
      linha('Local', h.nome) +
