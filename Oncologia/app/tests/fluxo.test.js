...
const agendaFalsa = {
  eventos: [],
  ocupadosExtras: [],
  reset() { this.eventos = []; this.ocupadosExtras = []; },

  async ocupados() {
    return [
      ...this.ocupadosExtras,
      ...this.eventos.map((e) => ({ inicio: e.start.dateTime, fim: e.end.dateTime })),
    ];
  },
  async criarPreAgendamento({ calendarId, titulo, inicioRFC, fimRFC, descricao, protocolo }) {
    const evento = {
      id: `evt-${this.eventos.length + 1}`,
      calendarId,
      summary: titulo,
  ...
  async buscarPorProtocolo(_ids, protocolo) {
    const evento = this.eventos.find((e) => e.extendedProperties.private.protocolo === protocolo);
  ...
  async pendentes(calendarId) {
    ...
    return this.eventos.filter((e) => e.calendarId === calendarId && e.status === 'tentative');
  },
};
...
beforeEach(() => { agendaFalsa.reset(); waFalso.reset(); });
...
test('agendar cria o evento provisório na agenda do hospital escolhido', async () => {
  ...
  assert.equal(agendaFalsa.eventos.length, 1);
  const evento = agendaFalsa.eventos[0];
...
test('formulário inválido não chega a criar evento', async () => {
  ...
  assert.equal(agendaFalsa.eventos.length, 0);
...
test('comando vindo de outro número é ignorado', async () => {
  ...
  assert.equal(agendaFalsa.eventos[0].status, 'tentative');   // nada mudou
});
...
test('falha ao avisar a recepção não desfaz o agendamento', async () => {
  ...
  try {
    ...
    assert.equal(agendaFalsa.eventos.length, 1);      // o horário continua reservado
  } finally {
...
test('cobrança das 24h só pega o que está parado além do prazo', async () => {
  await servico.agendar(PEDIDO, AGORA);
  agendaFalsa.eventos[0].created = new Date(Date.now() - 30 * 3600000).toISOString();
